...
def build_empty_buffer(ctx: mgl.Context, mesh: Mesh, size: int, update_vertices: bool = True) -> None:
    if update_vertices: 
        mesh.vertices = []
...
def update_vertex_buffer(mesh: Mesh, vertices: list[float], update_vertices: bool = True) -> None:
    size = len(vertices) * 4
    if size > mesh.size:
...
def append_vertex_buffer(mesh: Mesh, vertices: list[float], update_vertices: bool = True) -> None:
...
def rebuild_vertex_buffer(ctx: mgl.Context, mesh: Mesh, vertices: list[float], update_vertices: bool = True) -> None:
    if update_vertices:
        mesh.vertices = vertices
